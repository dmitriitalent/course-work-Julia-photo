...
export default {
    ...
    mounted() {
        ...
        axios.get(cfg.serverUrl + "api/photo/50").then(response => {
            this.photoList = [...this.cut([...this.photoList.flat(), ...response.data], this.columns)];
            console.log(this.photoList)
        });
        
        window.addEventListener("scroll", event => {
            if (document.getElementsByTagName("html")[0].offsetHeight - window.scrollY - window.innerHeight - 300 <= 0)
            {
                axios.get(cfg.serverUrl + "api/photo/40").then(response => {
                    this.photoList = this.append(this.photoList, response.data)
                });
            };
        })
    }
